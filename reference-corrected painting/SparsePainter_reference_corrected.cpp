// please compile with "make", or try the below command
// g++ -I./armadillo-12.6.5/include SparsePainter_dev.cpp -o SparsePainter -lz -fopenmp -lpthread -L./armadillo-12.6.5 -larmadillo -llapack -lblas -std=c++0x -g -O3 -Wl,-rpath=./armadillo-12.6.5
#ifdef _OPENMP
#include <omp.h>
#else
...
hMat indpainting(const hMat& mat,
  ...
  for(int j=0; j<nsnp; ++j){
    ...
    for(int k=0;k<margidx.size();++k){
      ...
      // update the probability of this 
      double probtemp=marginal_prob.m[j].get(refnumberidx);
      popprob[popidx] += probtemp;
      if(correct_painting){
        popprob2[popidx] += probtemp*refweight[refnumberidx][j];
      }
    }
    ...
    if(!correct_painting){
      ...
      double probsum=vec_sum(popprob2);
      if(probsum!=0){
        for(int i=0; i<npop; ++i){
          marginal_prob_pop.m[j].set(i,round(popprob2[i]/probsum* precision)/precision);
        }
      }else{
        for(int i=0; i<npop; ++i){
          marginal_prob_pop.m[j].set(i,round(popprob[i] * precision)/precision);
...
char convertASCII(double number){
  int toint=static_cast<int>(round(number*100));
  return static_cast<char>(33+toint);
}
...
#pragma omp parallel for reduction(+:lambda_sum)
  ...
  //begin calculating standardisation coancestry matrix if -weight is specified
  
  vector<vector<double>> coanc(npop, vector<double>(npop, 0.0));
  ...
  if(probstore=="weight"){
    ...
    while(nhap_left>0){
      ...
      cout<<"Calculating chunk length for reference haplotypes "<<nhap_use-nhap_left<<"-"<<nhap_use-nhap_left+nsamples_use-1<<endl;
...
#pragma omp parallel for
    ...
    cout<<"Begin calculating standardized coancestry matrix for weight"<<endl;
    for(int j=0;j<npop;++j){
      int nind_eachpop = refidx.findrows(j).size();
      if(!haploid){
        nind_eachpop/=2;
      }
      if(nind_eachpop==1){
        cerr << "Error: reference population " << j <<" do not have at least 2 samples" << endl;
        abort();
      }
      coanc[j][j] *= nind_eachpop / (nind_eachpop-1);
      
      double max_value = coanc[j][j];
      for (int ii = 0; ii < npop; ++ii) {
        if (coanc[ii][j] > max_value) {
          max_value = coanc[ii][j];
        }
      }
      for (int ii = 0; ii < npop; ++ii) {
        coanc[ii][j] /= max_value;
      }
    }
  }
  ...
  if(outputpainting && run!="chunk"){
    ...
    if(outputallSNP){
      ...
      }else if (probstore=="ASCII"){
        outputFile << "#Storage Mode: ASCII" <<"\n";
        outputFile << "SNPidx"<< " ";
        for (int j = 0; j < npop; ++j){
          outputFile << refmatch[j];
        }
...
#pragma omp parallel for
    for(int ii=nhap_use-nhap_left; ii<nhap_use-nhap_left+nsamples_use; ++ii){
      ...
      if(run!="chunk"){
        hMat pind=indpainting(mat,gd,lambda_use,npop,refindex,dp,f,b,correct_painting,refweight);
          ...
          if(probstore=="constant"){
            ...
            if(haploid){
              ...
              for(int ii=nhap_use-nhap_left; ii<nhap_use-nhap_left+nsamples_use; ++ii){
                ...
                outputFile << 1 <<" ";
                for(int k=0;k<npop;++k){
                  double val=painting_all[ii-nhap_use+nhap_left][k][0];
                  if(static_cast<int>(round(val*100))>=93){
                    char fix='~';
                    outputFile << fix;
                    val=val-0.93;
                  }
                  outputFile << convertASCII(val);
                  //if(k!=npop-1) outputFile << " ";
                }
                outputFile <<"\n";
...
#pragma omp parallel for reduction(+:rmse)
                  ...
                  if(rmse>rmsethre){
                    ...
                    outputFile << j <<" ";
                    for(int k=0;k<npop;++k){
                      double val=painting_all[ii-nhap_use+nhap_left][k][j-1];
                      if(static_cast<int>(round(val*100))>=93){
                        char fix='~';
                        outputFile << fix;
                        val=val-0.93;
                    ...
                    outputFile << j+1 <<" ";
                    for(int k=0;k<npop;++k){
                      double val=painting_all[ii-nhap_use+nhap_left][k][j];
                      if(static_cast<int>(round(val*100))>=93){
                        char fix='~';
                        outputFile << fix;
                        val=val-0.93;
              ...
              for(int ii=(nhap_use-nhap_left)/2; ii<(nhap_use-nhap_left+nsamples_use)/2; ++ii){
                ...
                outputFile << 1 <<" ";
                for(int k=0;k<npop;++k){
                  double val=painting_all[2*ii-nhap_use+nhap_left][k][0];
                  if(static_cast<int>(round(val*100))>=93){
                    char fix='~';
                    outputFile << fix;
                    val=val-0.93;
...
#pragma omp parallel for reduction(+:rmse)
                  ...
                  if(rmse>rmsethre){
                    ...
                    outputFile << j <<" ";
                    for(int k=0;k<npop;++k){
                      double val=painting_all[2*ii-nhap_use+nhap_left][k][j-1];
                      if(static_cast<int>(round(val*100))>=93){
                        char fix='~';
                        outputFile << fix;
                        val=val-0.93;
                    ...
                    outputFile << j+1 <<" ";
                    for(int k=0;k<npop;++k){
                      double val=painting_all[2*ii-nhap_use+nhap_left][k][j];
                      if(static_cast<int>(round(val*100))>=93){
                        char fix='~';
                        outputFile << fix;
                        val=val-0.93;
                ...
                outputFile << 1 <<" ";
                for(int k=0;k<npop;++k){
                  double val=painting_all[2*ii-nhap_use+nhap_left+1][k][0];
                  if(static_cast<int>(round(val*100))>=93){
                    char fix='~';
                    outputFile << fix;
                    val=val-0.93;
...
#pragma omp parallel for reduction(+:rmse)
                  ...
                  if(rmse>rmsethre){
                    ...
                    outputFile << j <<" ";
                    for(int k=0;k<npop;++k){
                      double val=painting_all[2*ii-nhap_use+nhap_left+1][k][j-1];
                      if(static_cast<int>(round(val*100))>=93){
                        char fix='~';
                        outputFile << fix;
                        val=val-0.93;
                    ...
                    outputFile << j+1 <<" ";
                    for(int k=0;k<npop;++k){
                      double val=painting_all[2*ii-nhap_use+nhap_left+1][k][j];
                      if(static_cast<int>(round(val*100))>=93){
                        char fix='~';
                        outputFile << fix;
                        val=val-0.93;
            ...
            if(haploid){
              for(int ii=nhap_use-nhap_left; ii<nhap_use-nhap_left+nsamples_use; ++ii){
                ...
                for (int j = 0; j < nsnp; ++j) {
                  ...
                  sum_weight=0;
                  for(int k=0;k<npop;++k){
                    sum_weight+=painting_all[ii-nhap_use+nhap_left][k][j]*coanc[k][selfpop];
                  }
                  sum_weight=round(sum_weight*precision)/precision;
                  outputFile << sum_weight;
                  if(j!=nsnp-1) outputFile << " ";
                }
                outputFile << "\n";
              }
              ...
              for(int ii=(nhap_use-nhap_left)/2; ii<(nhap_use-nhap_left+nsamples_use)/2; ++ii){
                ...
                for (int j = 0; j < nsnp; ++j) {
                  sum_weight=0;
                  for(int k=0;k<npop;++k){
                    sum_weight+=painting_all[2*ii-nhap_use+nhap_left][k][j]*coanc[k][selfpop];
                  }
                  sum_weight=round(sum_weight*precision)/precision;
                  outputFile << sum_weight;
                  if(j!=nsnp-1) outputFile << " ";
                ...
                for (int j = 0; j < nsnp; ++j) {
                  sum_weight=0;
                  for(int k=0;k<npop;++k){
                    sum_weight+=painting_all[2*ii-nhap_use+nhap_left+1][k][j]*coanc[k][selfpop];
                  }
                  sum_weight=round(sum_weight*precision)/precision;
                  outputFile << sum_weight;
                  if(j!=nsnp-1) outputFile << " ";
...
int main(int argc, char *argv[]){
  ...
  if (argc == 1 || string(argv[1]) == "-help" || string(argv[1]) == "-h") {
    ...
    cout << "SparsePainter reference: Yang, Y., Durbin, R., Iversen, A.K.N & Lawson, D.J. Sparse haplotype-based fine-scale local ancestry inference at scale reveals recent selection on immune responses. Nature Communications (2025)." << endl<< endl;
    ...
    cout << "  -reffile [file]: Reference vcf (including gzipped vcf), or phase (including gzipped phase) file that contains the (phased non-missing) genotype data for all the reference samples." << endl<< endl;
    ...
    cout << "  -weight: Output the weight of each reference sample at each SNP. Its output if the required input of [weightfile] to correct for local ancestry estimates." << endl<< endl;
    ...
    cout << "  -weightfile [file]: File contains the output of [-weight]. If this file is not specified (default), the raw local ancestry probabilities will be output; if this file is provided, the corrected local ancestry probabilities will be output." <<endl<< endl;
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if(param=="prob" || param=="chunklength" || param=="chunkcount" ||
       param=="aveSNP" || param=="aveind" || param=="outmatch" ||
    ...
    if (param == "weight") {
      ...
      outputpainting=true;
      if(probstore=="weight"){
        cerr<<"Warning: Only the weight file will be output. Ignore -prob."<<endl;
      }
      ...
      if(probstore=="weight"){
        cerr<<"Warning: Only the weight file will be output. Ignore -probstore "<<argv[++i]<<"."<<endl;
      }else{
        probstore = argv[++i];
      }
  ...
  if (!((reffile_vcf && targetfile_vcf) || (reffile_phase && targetfile_phase))) {
    cerr << "Warning: The reffile and targetfile should both be vcf (including gzipped vcf) or phase (including gzipped phase) format." << endl;
    cerr<<"Type -h or -help to see the help file."<<endl;
    return 1;
  }
  ...
  if(!runpaint && !clength && !ccount){
    ...
    cerr<<"-weight: output the weight of each haplotype at each SNP for painting correction."<<endl;
    cerr<<"Type -h or -help to see the help file."<<endl;
    return 1;
  }
  ...
  if(probstore!="raw" && probstore!="constant" && probstore!="ASCII" && probstore!="linear" && probstore!="cluster" && probstore!="weight"){
  ...
  if(probstore=="weight" && reffile!=targetfile){
    cerr<<"Error: -weight is only valid under reference-vs-reference painting. Please ensure your -reffile is the same as -targetfile."<<endl;
    abort();
  }
  ...
  paintall(method, diff_lambda, fixlambda, EM_ite, indfrac, minsnpEM, EMsnpfrac, L_initial, nmatch, L_minmatch, haploid, 
           leaveoneout, reffile, targetfile, mapfile, popfile, namefile, matchfile, weightfile, SNPfile,nmatchfile, 
